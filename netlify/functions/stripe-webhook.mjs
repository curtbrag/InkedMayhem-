  ...
  for (const m of milestones) {
    if (totalUsers === m) {
      await sendTelegramNotification(
        `\u{1F389} <b>MILESTONE!</b>

You just hit <b>${m} total members!</b>
Keep growing! \u{1F680}`
      );
    }
    if (paidCount === m) {
      await sendTelegramNotification(
        `\u{1F4B0} <b>MILESTONE!</b>

You now have <b>${m} paying subscribers!</b>
The hustle is paying off! \u{1F525}`
      );
    }
  }
}
var stripe_webhook_default = async (req, context) => {
  if (req.method !== "POST") {
  ...
  const stripe = new stripe_esm_node_default(stripeKey);
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      if (session.mode === "subscription") {
        ...
        await sendTelegramNotification(
          `\u{1F525} <b>New Subscriber!</b>

<b>${user.name || email}</b> just joined <b>${tierNames[tier] || tier.toUpperCase()}</b>!
\u{1F4B0} ${session.amount_total ? `$${(session.amount_total / 100).toFixed(2)}/mo` : "N/A"}`
        );
        try {
          await checkMilestones(store);
        } catch {
        }
      } else {
...
Attempt: ${invoice.attempt_count || 1}`
    ...
    case "customer.subscription.deleted": {
      ...
      if (found) {
        ...
        found.user.cancelledAt = (/* @__PURE__ */ new Date()).toISOString();
        ...
        await sendTelegramNotification(
          `\u{1F4C9} <b>Subscription Cancelled</b>
    ...
    case "customer.subscription.updated": {
      ...
      if (found) {
        if (sub.cancel_at_period_end) {
          ...
          found.user.cancelAt = sub.cancel_at ? new Date(sub.cancel_at * 1e3).toISOString() : null;
        } else {
...
Amount: $${((charge.amount_refunded || 0) / 100).toFixed(2)}`
    ...
    case "charge.dispute.created": {
      ...
      await sendTelegramNotification(
        `\u{1F6A8} <b>DISPUTE / CHARGEBACK!</b>
