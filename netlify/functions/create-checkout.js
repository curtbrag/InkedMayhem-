...
__export(create_checkout_exports, {
  config: () => config,
  default: () => create_checkout_default
});
...
var DEFAULT_POST_PRICE = 499;
var create_checkout_default = async (req, context) => {
  ...
  try {
    ...
    if (promoCode) {
      try {
        ...
        if (promo && promo.active && (!promo.expiresAt || promo.expiresAt > (/* @__PURE__ */ new Date()).toISOString())) {
    ...
    if (type === "subscription" && TIERS[tier]) {
      ...
      if (promoCode && discounts.length) {
        try {
          ...
          if (promo) {
            ...
            promo.usedBy.push({ email: decoded.email, at: (/* @__PURE__ */ new Date()).toISOString() });
      ...
      try {
        ...
        if (user?.purchases?.some((p) => p.postId === postId)) {
