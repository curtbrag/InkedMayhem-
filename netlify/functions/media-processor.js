...
var media_processor_exports = {};
__export(media_processor_exports, {
  config: () => config,
  default: () => media_processor_default,
  processImage: () => processImage
});
module.exports = __toCommonJS(media_processor_exports);
...
var media_processor_default = async (req, context) => {
  ...
  const expectedKey = process.env.PIPELINE_API_KEY || process.env.JWT_SECRET || "inkedmayhem-dev-secret-change-me";
  if (apiKey !== expectedKey) {
    ...
    try {
      const jwt = await Promise.resolve().then(() => __toESM(require_jsonwebtoken(), 1));
      const decoded = jwt.default.verify(auth.replace("Bearer ", ""), expectedKey);
      if (!decoded.isAdmin) {
        return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: CORS });
      }
    } catch {
  ...
  try {
    ...
    const savings = result.originalSizeBytes > 0 ? Math.round((1 - result.processedSizeBytes / result.originalSizeBytes) * 100) : 0;
